"""
Wind3 Reload Dashboard — v5.1
Autore: Salvo (Ambassador A6 — Campania & Puglia)

Novità v5.1:
- Indirizzo (STORE_ADDRESS) presente in tutti i report: Excel DM/AM, drill-down RS, ricerca
- Forever NUMERICO: somma reale R4 Basic+Premium (gross/net), non più solo flag 0/1
- Anagrafica RS arricchita: lista negozi, città, indirizzi, codici PDV concatenati
- Banner "X punti vendita" nel drill-down RS per evidenziare PDV multipli
- Autocomplete con SHOP_CODE in label → niente più duplicati persi nel dict
- Header Excel/PDF con range cella dinamico (non più hardcoded A1:N1)
- Indici colorazione Gross%/Net% calcolati dinamicamente per resilienza

Novità v5:
- Fix CSS: sidebar visibile su desktop e mobile (era nascosta dal display:none header)
- Vettorizzazione: 10-50× più veloce su zone grandi (rimossi tutti i .apply axis=1 con safe_pct)
- Cache su compute_kpis, compute_rs_aggregates, compute_compensi_reload, build_hierarchy
- Render DM cards in unico st.markdown (no più 27 chiamate separate)
- Bulk WhatsApp: genera messaggi per TUTTI i DM in un .txt o ZIP
- Ricerca con autocomplete: typeahead su elenco completo store
- Tab "🧾 Genera Chiusure" via modulo chiusure.py
...
from chiusure import render_chiusure_tab
...
.dm-forever { font-size:0.7rem; color:var(--muted); text-align:center; min-width:108px; line-height:1.35; }
...
# ─────────────────────────────────────────────────────────────────
#  VECTORIZED HELPERS (10-50x più veloci dei .apply axis=1)
# ─────────────────────────────────────────────────────────────────
def safe_pct_vec(num, den):
    """
    Versione vettoriale di safe_pct per Series pandas.
    Restituisce 0.0 dove den è 0/NaN, altrimenti round(num/den*100, 1).
    Supporta anche denominatore scalare.
    """
    import numpy as np
    n = pd.to_numeric(num, errors="coerce").fillna(0).astype(float)
    # Se den è scalare, costruisci serie costante
    if not isinstance(den, pd.Series):
        try:
            d_val = float(den) if den is not None and not (isinstance(den, float) and pd.isna(den)) else 0
        except (TypeError, ValueError):
            d_val = 0
        if d_val == 0:
            return pd.Series(0.0, index=n.index)
        return ((n / d_val) * 100.0).round(1).fillna(0.0)
    # Denominatore Series: usa np.nan (non pd.NA) per arithmetic safety
...
def gross_pct_vec(gross, tam, forever, mode="standard"):
    """Versione vettoriale di gross_pct_calc."""
    if mode == "adjusted":
        return safe_pct_vec(
            pd.to_numeric(gross, errors="coerce").fillna(0)
            + pd.to_numeric(forever, errors="coerce").fillna(0),
            pd.to_numeric(tam, errors="coerce").fillna(0)
            + pd.to_numeric(forever, errors="coerce").fillna(0),
        )
    return safe_pct_vec(gross, tam)


def net_pct_vec(net, tam, forever, mode="standard"):
    """Versione vettoriale di net_pct_calc."""
    if mode == "adjusted":
        return safe_pct_vec(
            pd.to_numeric(net, errors="coerce").fillna(0)
            + pd.to_numeric(forever, errors="coerce").fillna(0),
            pd.to_numeric(tam, errors="coerce").fillna(0)
            + pd.to_numeric(forever, errors="coerce").fillna(0),
        )
    return safe_pct_vec(net, tam)
...
def compute_kpis(df, df_prev, cfg):
    ...
    [v5] Vettorizzato: 10-50x più veloce della v4 su zone grandi."""
    if df.empty:
        return pd.DataFrame()

    calc_mode = cfg.get("calc_mode", "standard")

    # [v5.1] Forever numeric: somma reale di R4_BASIC + R4_PREMIUM (gross & net)
    # Activeforever resta come flag 0/1 per il conteggio "stores con almeno una Forever"
    has_r4 = all(c in df.columns for c in ["R4_BASIC_GROSS_SALES", "R4_PREMIUM_GROSS_SALES"])
    agg_dict = dict(
        TAM=("TAM", "sum"),
    ...
    if has_r4:
        agg_dict.update(dict(
            Forever_Basic_Gross=("R4_BASIC_GROSS_SALES", "sum"),
            Forever_Premium_Gross=("R4_PREMIUM_GROSS_SALES", "sum"),
            Forever_Basic_Net=("R4_BASIC_NET_SALES", "sum"),
            Forever_Premium_Net=("R4_PREMIUM_NET_SALES", "sum"),
        ))

    agg = df.groupby(["REGION", "AREA_MANAGER", "DISTRICT_MANAGER"], dropna=False).agg(**agg_dict).reset_index()

    if has_r4:
        agg["Forever_Tot_Gross"] = agg["Forever_Basic_Gross"] + agg["Forever_Premium_Gross"]
        agg["Forever_Tot_Net"]   = agg["Forever_Basic_Net"]   + agg["Forever_Premium_Net"]
    else:
        agg["Forever_Basic_Gross"] = 0
        agg["Forever_Premium_Gross"] = 0
        agg["Forever_Basic_Net"] = 0
        agg["Forever_Premium_Net"] = 0
        agg["Forever_Tot_Gross"] = 0
        agg["Forever_Tot_Net"] = 0

    # Calcoli STANDARD e ADJUSTED — vettorizzati
    agg["Gross_%_std"] = safe_pct_vec(agg["Gross_Sales"], agg["TAM"])
    agg["Net_%_std"]   = safe_pct_vec(agg["Net_Sales"], agg["TAM"])
    agg["Gross_%_adj"] = gross_pct_vec(agg["Gross_Sales"], agg["TAM"], agg["Forever_Active"], "adjusted")
    agg["Net_%_adj"]   = net_pct_vec(agg["Net_Sales"], agg["TAM"], agg["Forever_Active"], "adjusted")

    # Le colonne "ufficiali" cambiano in base alla modalità
    ...
    agg["Forever_%"] = safe_pct_vec(agg["Forever_Active"], agg["Total_Stores"])
    ...
    if df_prev is not None and not df_prev.empty:
        ...
        if calc_mode == "adjusted":
            pa["Gross_%_p"] = gross_pct_vec(pa["Gross_p"], pa["TAM_p"], pa["Forever_p"], "adjusted")
            pa["Net_%_p"]   = net_pct_vec(pa["Net_p"], pa["TAM_p"], pa["Forever_p"], "adjusted")
        else:
            pa["Gross_%_p"] = safe_pct_vec(pa["Gross_p"], pa["TAM_p"])
            pa["Net_%_p"]   = safe_pct_vec(pa["Net_p"], pa["TAM_p"])
    ...
    # Status & alerts vettorizzati
    gross_below_crit = agg["Gross_%"] < sg
    net_below_crit   = agg["Net_%"] < sn
    forever_zero     = agg["Forever_Active"] == 0
    gross_below_att  = agg["Gross_%"] < sa_g
    net_below_att    = agg["Net_%"] < sa_n

    is_critical = gross_below_crit | net_below_crit | (forever_zero & forever_zero_critico)
    is_warn     = (~is_critical) & (gross_below_att | net_below_att | forever_zero)

    agg["status"] = "ok"
    agg.loc[is_warn, "status"] = "attenzione"
    agg.loc[is_critical, "status"] = "critico"

    # alerts: una list per riga (questo serve un loop, ma è solo string concat)
    alerts_list = []
    for _, r in agg.iterrows():
        a = []
        if r["Gross_%"] < sg: a.append(f"Gross {r['Gross_%']}% < {sg}%")
        if r["Net_%"]   < sn: a.append(f"Net {r['Net_%']}% < {sn}%")
        if r["Forever_Active"] == 0:
            a.append("Forever = 0" if forever_zero_critico else "Forever = 0 (warn)")
        alerts_list.append(a)
    agg["alerts"] = alerts_list

    # Trend vettorizzato
    def _tl_vec(s):
        out = pd.Series("—", index=s.index, dtype=object)
        valid = s.notna()
        out[valid & (s > 0)] = "▲ +" + s[valid & (s > 0)].astype(str) + "%"
        out[valid & (s < 0)] = "▼ " + s[valid & (s < 0)].astype(str) + "%"
        out[valid & (s == 0)] = "= 0%"
        return out

    agg["Gross_Trend"] = _tl_vec(agg["Gross_Δ"])
    agg["Net_Trend"]   = _tl_vec(agg["Net_Δ"])
...
def analyze_gross_net(df, soglia_gross, soglia_net, sa_gross, sa_net):
    ...
    # Per-store metrics
    store_df = df.copy()
    store_df["g_pct"] = safe_pct_vec(store_df["SR_PLUS_GROSS_SALES"], store_df["TAM"])
    store_df["n_pct"] = safe_pct_vec(store_df["SR_PLUS_NET_SALES"], store_df["TAM"])
    ...
    by_type["gross_pct"] = safe_pct_vec(by_type["gross"], by_type["tam"])
    by_type["net_pct"] = safe_pct_vec(by_type["net"], by_type["tam"])
    by_type["share_gross"] = safe_pct_vec(by_type["gross"], gross)
...
def generate_message(row, meta):
    ...
    # [v5.1] Forever numerico — Basic + Premium (gross/net)
    fb_g = int(row.get("Forever_Basic_Gross", 0) or 0)
    fp_g = int(row.get("Forever_Premium_Gross", 0) or 0)
    fb_n = int(row.get("Forever_Basic_Net", 0) or 0)
    fp_n = int(row.get("Forever_Premium_Net", 0) or 0)
    ftot_g = fb_g + fp_g
    ftot_n = fb_n + fp_n

    forever_detail = (
        f"   • Basic: {fb_g} gross / {fb_n} net\n"
        f"   • Premium: {fp_g} gross / {fp_n} net\n"
        f"   • *TOTALE: {ftot_g} gross / {ftot_n} net*"
        if ftot_g > 0 or ftot_n > 0
        else "   • Nessuna Forever venduta"
    )
...
def compute_rs_aggregates(df, calc_mode="standard"):
    ...
    # Anagrafica derivata: AM, DM, regioni, store types, INDIRIZZI, STORE CODES
    def _join_unique(s):
        return ", ".join(sorted(set(safe_str(x) for x in s if pd.notna(x) and safe_str(x) not in ("", "—"))))

    anagrafica = df.groupby("COMPANY_NAME", dropna=False).agg(
        AM=("AREA_MANAGER", _join_unique),
        DM=("DISTRICT_MANAGER", _join_unique),
        Regioni=("REGION", _join_unique),
        Tipi_Store=("STORE_TYPE", _join_unique),
        Province=("PROVINCE_CODE", _join_unique),
        # [v5.1] Lista negozi e indirizzi: serve per non perdere PDV multipli
        Negozi=("STORE", _join_unique),
        Citta=("CITY", _join_unique),
        Indirizzi=("STORE_ADDRESS", _join_unique) if "STORE_ADDRESS" in df.columns else ("CITY", lambda _: ""),
        Shop_Codes=("SHOP_CODE", lambda s: ", ".join(sorted(set(safe_str(x) for x in s if pd.notna(x))))),
    ).reset_index()
    rs = rs.merge(anagrafica, on="COMPANY_NAME", how="left")

    # [v5.1] Forever NUMERICO: somma reale R4 Basic+Premium (gross/net), invece del flag 0/1
    rs["Forever_Tot_Gross"] = rs.get("r4_basic_gross", 0) + rs.get("r4_premium_gross", 0)
    rs["Forever_Tot_Net"]   = rs.get("r4_basic_net", 0) + rs.get("r4_premium_net", 0)

    # KPI calcolati — vettorizzati
    rs["Gross_%_std"] = safe_pct_vec(rs["Gross"], rs["TAM"])
    rs["Net_%_std"]   = safe_pct_vec(rs["Net"], rs["TAM"])
    rs["Gross_%_adj"] = gross_pct_vec(rs["Gross"], rs["TAM"], rs["Forever_Active"], "adjusted")
    rs["Net_%_adj"]   = net_pct_vec(rs["Net"], rs["TAM"], rs["Forever_Active"], "adjusted")
    ...
    rs["Forever_%"] = safe_pct_vec(rs["Forever_Active"], rs["Total_Stores"])
...
def build_excel_dm(row, df_raw, meta, soglia_gross, soglia_net, sa_gross, sa_net,
    ...
    sub["Gross_%"] = safe_pct_vec(sub["SR_PLUS_GROSS_SALES"], sub["TAM"])
    sub["Net_%"] = safe_pct_vec(sub["SR_PLUS_NET_SALES"], sub["TAM"])
    # [v5.1] Forever NUMERICO per store: Basic+Premium gross e net
    if "R4_BASIC_GROSS_SALES" in sub.columns:
        sub["Forever_Tot_Gross"] = (
            pd.to_numeric(sub["R4_BASIC_GROSS_SALES"], errors="coerce").fillna(0)
            + pd.to_numeric(sub["R4_PREMIUM_GROSS_SALES"], errors="coerce").fillna(0)
        )
        sub["Forever_Tot_Net"] = (
            pd.to_numeric(sub["R4_BASIC_NET_SALES"], errors="coerce").fillna(0)
            + pd.to_numeric(sub["R4_PREMIUM_NET_SALES"], errors="coerce").fillna(0)
        )
    else:
        sub["Forever_Tot_Gross"] = 0
        sub["Forever_Tot_Net"] = 0
    ...
    store_cols = ["STORE", "COMPANY_NAME", "STORE_ADDRESS", "CITY", "PROVINCE_CODE", "STORE_TYPE", "TAM",
                  "SR_PLUS_GROSS_SALES", "Gross_%", "SR_PLUS_NET_SALES", "Net_%",
                  "Activeforever", "Forever_Tot_Gross", "Forever_Tot_Net", "TotalStore"]
    # Solo le colonne presenti
    store_cols = [c for c in store_cols if c in sub.columns]
    # Aggiungi colonne opzionali se presenti
    extra_store = []
    for ec in ["PLUS_GROSS_SALES", "EXCHANGE_SALES"]:
        if ec in sub.columns:
            store_cols.append(ec)
            extra_store.append(ec)

    hdrs2_map = {
        "STORE": "Store", "COMPANY_NAME": "Rag. Sociale", "STORE_ADDRESS": "Indirizzo",
        "CITY": "Città", "PROVINCE_CODE": "Prov", "STORE_TYPE": "Tipo", "TAM": "TAM",
        "SR_PLUS_GROSS_SALES": "Gross", "Gross_%": "Gross%",
        "SR_PLUS_NET_SALES": "Net", "Net_%": "Net%",
        "Activeforever": "Forever (0/1)", "Forever_Tot_Gross": "Forever Tot G",
        "Forever_Tot_Net": "Forever Tot N", "TotalStore": "Tot Store",
        "PLUS_GROSS_SALES": "Plus", "EXCHANGE_SALES": "Exchange",
    }
    hdrs2 = [hdrs2_map.get(c, c) for c in store_cols]

    # [v5.1] Header merge dinamico in base al numero effettivo di colonne
    n_cols = len(hdrs2)
    ...
    # [v5.1] Indici colonne dinamici (variano in base alle colonne effettivamente presenti)
    gross_idx = store_cols.index("Gross_%") + 1 if "Gross_%" in store_cols else None
    net_idx   = store_cols.index("Net_%") + 1 if "Net_%" in store_cols else None
    for ri, (_, sr) in enumerate(sub_sorted[store_cols].iterrows(), 3):
        for ci, val in enumerate(sr.values, 1):
            ...
            if gross_idx and ci == gross_idx and isinstance(val, (int, float)):
                fill = _color_for_pct(val, soglia_gross, sa_gross, RED, ORANGE, GREEN)
                if fill: c.fill = fill
            if net_idx and ci == net_idx and isinstance(val, (int, float)):
...
def build_excel_am(am_name, kpis, df_raw, meta, soglia_gross, soglia_net, sa_gross, sa_net,
    ...
    am_stores["Gross_%"] = safe_pct_vec(am_stores["SR_PLUS_GROSS_SALES"], am_stores["TAM"])
    am_stores["Net_%"] = safe_pct_vec(am_stores["SR_PLUS_NET_SALES"], am_stores["TAM"])
    # [v5.1] Forever numerico
    if "R4_BASIC_GROSS_SALES" in am_stores.columns:
        am_stores["Forever_Tot_Gross"] = (
            pd.to_numeric(am_stores["R4_BASIC_GROSS_SALES"], errors="coerce").fillna(0)
            + pd.to_numeric(am_stores["R4_PREMIUM_GROSS_SALES"], errors="coerce").fillna(0)
        )
        am_stores["Forever_Tot_Net"] = (
            pd.to_numeric(am_stores["R4_BASIC_NET_SALES"], errors="coerce").fillna(0)
            + pd.to_numeric(am_stores["R4_PREMIUM_NET_SALES"], errors="coerce").fillna(0)
        )
    else:
        am_stores["Forever_Tot_Gross"] = 0
        am_stores["Forever_Tot_Net"] = 0

    # [v5.1] Aggiungi STORE_ADDRESS + Forever numerici
    st_cols = ["DISTRICT_MANAGER", "STORE", "COMPANY_NAME", "STORE_ADDRESS", "CITY", "PROVINCE_CODE",
               "STORE_TYPE", "TAM", "SR_PLUS_GROSS_SALES", "Gross_%",
               "SR_PLUS_NET_SALES", "Net_%", "Activeforever",
               "Forever_Tot_Gross", "Forever_Tot_Net"]
    st_cols = [c for c in st_cols if c in am_stores.columns]
    st_hdrs_map = {
        "DISTRICT_MANAGER": "DM", "STORE": "Store", "COMPANY_NAME": "Rag. Sociale",
        "STORE_ADDRESS": "Indirizzo", "CITY": "Città", "PROVINCE_CODE": "Prov",
        "STORE_TYPE": "Tipo", "TAM": "TAM",
        "SR_PLUS_GROSS_SALES": "Gross", "Gross_%": "Gross%",
        "SR_PLUS_NET_SALES": "Net", "Net_%": "Net%",
        "Activeforever": "Forever (0/1)",
        "Forever_Tot_Gross": "Forever Tot G", "Forever_Tot_Net": "Forever Tot N",
    }
    st_hdrs = [st_hdrs_map.get(c, c) for c in st_cols]

    # [v5.1] Header merge dinamico
    n_cols3 = len(st_hdrs)
    last_col3 = get_column_letter(n_cols3)
    ws3.merge_cells(f"A1:{last_col3}1")
    tc3 = ws3["A1"]
    ...
    # [v5.1] Indici dinamici
    g_idx3 = st_cols.index("Gross_%") + 1 if "Gross_%" in st_cols else None
    n_idx3 = st_cols.index("Net_%") + 1 if "Net_%" in st_cols else None
    for ri, (_, sr) in enumerate(am_sorted_stores[st_cols].iterrows(), 3):
        for ci, val in enumerate(sr.values, 1):
            c = ws3.cell(row=ri, column=ci, value=val)
            c.border = BD; c.alignment = C
            if g_idx3 and ci == g_idx3 and isinstance(val, (int, float)):
                fill = _color_for_pct(val, soglia_gross, sa_gross, RED, ORANGE, GREEN)
                if fill: c.fill = fill
            if n_idx3 and ci == n_idx3 and isinstance(val, (int, float)):
                fill = _color_for_pct(val, soglia_net, sa_net, RED, ORANGE, GREEN)
                if fill: c.fill = fill
        ws3.row_dimensions[ri].height = 17
...
<div class="w3-header">
    ...
    <div class="w3-sub">Sales Performance Dashboard · v5.1</div>
...
#  TABS
# ─────────────────────────────────────────────────────────────────
tab1, tab2, tab3, tab_rs, tab_comp, tab4, tab_chiusure = st.tabs([
    "📋 Riepilogo DM",
    "📄 Dettaglio & Messaggi",
    "🌳 Mappa Zona",
    "🏢 Ragioni Sociali",
...
with tab1:
    ...
    if filtro_tipo != "Tutti":
        ...
        at["Gross_%_t"] = safe_pct_vec(at["Gross_t"], at["TAM_t"])
        at["Net_%_t"] = safe_pct_vec(at["Net_t"], at["TAM_t"])
    ...
    # [v5] Render TUTTE le card in un unico st.markdown (10x più veloce)
    if not filtered.empty:
        ...
        for _idx, row in filtered.iterrows():
            ...
            # [v5.1] Forever numerico
            f_tot_gross = int(row.get("Forever_Tot_Gross", 0) or 0)
            f_tot_net   = int(row.get("Forever_Tot_Net", 0) or 0)
            ...
            <div class="dm-card {row['status']}">
                ...
                <div class="dm-forever">
                    ...
                    <span style="font-size:0.78rem">{row['Forever_%']}%</span><br>
                    <span style="font-size:0.74rem;color:var(--accent);font-weight:600">
                        🎯 {f_tot_gross}G / {f_tot_net}N
                    </span>
                </div>
        ...
        # Selettore unico per drill-down (sostituisce 27 bottoni)
        st.markdown(" ")
        col_dd1, col_dd2 = st.columns([3, 1])
        with col_dd1:
            dm_drill = st.selectbox(
                "🏪 Vai al dettaglio store di:",
                ["— seleziona —"] + filtered["DISTRICT_MANAGER"].tolist(),
                key="dm_drill_select",
            )
        with col_dd2:
            st.write("")
            st.write("")
            if st.button("Vai", key="btn_drill_dm", use_container_width=True,
                         disabled=(dm_drill == "— seleziona —")):
                st.session_state["selected_dm"] = dm_drill
...
with tab2:
    ...
    with sub_tab1:
        # ── BULK WHATSAPP — genera messaggi per tutti i DM in un colpo ──
        with st.expander("📤 **Genera messaggi per TUTTI i DM** (bulk)", expanded=False):
            st.caption(
                "Genera in un colpo i messaggi WhatsApp di tutti i DM filtrati. "
                "Scarica come singolo file di testo (tutti i messaggi separati da `---`) "
                "o come ZIP con un .txt per DM."
            )
            ...
            with col_bulk1:
                bulk_filter = st.selectbox(
                    "Quali DM includere?",
                    ["Tutti", "🔴 Solo Critici", "⚠️ Solo Attenzione", "✅ Solo OK", "🔴+⚠️ Critici+Attenzione"],
                    key="bulk_msg_filter",
                )
            with col_bulk2:
                bulk_sort = st.selectbox(
                    "Ordina per",
                    ["Status (peggiori prima)", "Nome A-Z", "Gross % asc", "Gross % desc"],
                    key="bulk_msg_sort",
                )

            # Applica filtro
            kpis_bulk = kpis.copy()
            if bulk_filter == "🔴 Solo Critici":
                kpis_bulk = kpis_bulk[kpis_bulk["status"] == "critico"]
            elif bulk_filter == "⚠️ Solo Attenzione":
                kpis_bulk = kpis_bulk[kpis_bulk["status"] == "attenzione"]
            elif bulk_filter == "✅ Solo OK":
                kpis_bulk = kpis_bulk[kpis_bulk["status"] == "ok"]
            elif bulk_filter == "🔴+⚠️ Critici+Attenzione":
                kpis_bulk = kpis_bulk[kpis_bulk["status"].isin(["critico", "attenzione"])]

            # Applica sort
            if bulk_sort == "Status (peggiori prima)":
                status_order = {"critico": 0, "attenzione": 1, "ok": 2}
                kpis_bulk = kpis_bulk.copy()
                kpis_bulk["_so"] = kpis_bulk["status"].map(status_order).fillna(99)
                kpis_bulk = kpis_bulk.sort_values(["_so", "Gross_%"], ascending=[True, True]).drop(columns=["_so"])
            elif bulk_sort == "Nome A-Z":
                kpis_bulk = kpis_bulk.sort_values("DISTRICT_MANAGER")
            elif bulk_sort == "Gross % asc":
                kpis_bulk = kpis_bulk.sort_values("Gross_%", ascending=True)
            else:
                kpis_bulk = kpis_bulk.sort_values("Gross_%", ascending=False)

            st.markdown(f"**{len(kpis_bulk)} DM** da processare")

            if not kpis_bulk.empty:
                # Genera tutti i messaggi
                all_messages = []
                msg_per_dm = {}
                for _, r in kpis_bulk.iterrows():
                    m = generate_message(r, meta)
                    all_messages.append(m)
                    msg_per_dm[r["DISTRICT_MANAGER"]] = m

                separator = "\n\n" + "═" * 40 + "\n\n"
                combined = separator.join(all_messages)

                # Anteprima
                st.text_area(
                    "Anteprima (tutti i messaggi)",
                    combined,
                    height=240,
                    key="bulk_preview",
                )
                ...
                with col_dl1:
                    st.download_button(
                        "⬇️ Scarica TUTTI in un .txt",
                        data=combined.encode("utf-8"),
                        file_name=f"messaggi_TUTTI_{zona_label}_{meta['month']:02d}_{meta['year']}.txt",
                        mime="text/plain",
                        use_container_width=True,
                        key="dl_bulk_combined",
                    )
                with col_dl2:
                    # ZIP con un .txt per DM
                    zip_buf = io.BytesIO()
                    with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
                        for dm, m in msg_per_dm.items():
                            safe_name = "".join(c if c.isalnum() else "_" for c in str(dm))
                            zf.writestr(
                                f"{safe_name}_{meta['month']:02d}_{meta['year']}.txt",
                                m,
                            )
                    zip_buf.seek(0)
                    st.download_button(
                        "🗂️ Scarica ZIP (1 file per DM)",
                        data=zip_buf.getvalue(),
                        file_name=f"messaggi_DM_{zona_label}_{meta['month']:02d}_{meta['year']}.zip",
    ...
    with sub_tab2:
        ...
        # [v5] Modalità ricerca: testo libero o autocomplete da elenco
        search_mode = st.radio(
            "Modalità",
            ["📝 Testo libero (multi-campo)", "💡 Autocomplete (digita o seleziona)"],
            horizontal=True,
            label_visibility="collapsed",
            key="search_mode",
        )
        ...
        if search_mode == "💡 Autocomplete (digita o seleziona)":
            # [v5.1] Costruisci elenco con SHOP_CODE per garantire unicità anche
            # quando 2 store hanno stessa STORE+RS+CITY (es. due punti nella stessa via)
            df_idx = df.copy().reset_index(drop=True)
            addr_part = ""
            if "STORE_ADDRESS" in df_idx.columns:
                addr_part = df_idx["STORE_ADDRESS"].astype(str)
            else:
                addr_part = pd.Series([""] * len(df_idx))
            labels = (
                "[" + df_idx["SHOP_CODE"].astype(str) + "] "
                + df_idx["STORE"].astype(str)
                + " — " + df_idx["COMPANY_NAME"].astype(str)
                + " — " + addr_part
                + " · " + df_idx["CITY"].astype(str)
                + " (" + df_idx["STORE_TYPE"].astype(str) + ")"
            )
            # Lista (non dict) per non perdere righe duplicate per label
            label_list = labels.tolist()
            options_map = list(zip(label_list, df_idx.index.tolist()))
            with col_sq:
                pick = st.selectbox(
                    "Cerca",
                    ["— digita per cercare —"] + sorted(label_list),
                    key="autocomplete_pick",
                    label_visibility="collapsed",
                )
            with col_st1:
                ft_search = st.selectbox(
                    "Tipo", ["Tutti"] + sorted(df["STORE_TYPE"].dropna().unique().tolist()),
                    key="tipo_search_ac", label_visibility="collapsed",
                )
            with col_st2:
                ft_region_search = st.selectbox(
                    "Regione", ["Tutte"] + sorted(df["REGION"].dropna().unique().tolist()),
                    key="reg_search_ac", label_visibility="collapsed",
                )
            search_query = ""
            results = pd.DataFrame()
            if pick != "— digita per cercare —":
                # [v5.1] Recupera l'indice dal primo match (label_list può avere duplicati teorici)
                idx = None
                for lbl, i in options_map:
                    if lbl == pick:
                        idx = i
                        break
                if idx is not None:
                    results = df_idx.iloc[[idx]].copy()
                    if ft_search != "Tutti":
        ...
        # Render risultati (comune ai due rami)
        if (search_mode.startswith("📝") and search_query.strip()) or \
           (search_mode.startswith("💡") and not results.empty):

            if results.empty:
                st.info(f"Nessun risultato per: {search_query}" if search_query else "Nessun risultato con questi filtri")
            else:
                results["Gross_%"] = safe_pct_vec(results["SR_PLUS_GROSS_SALES"], results["TAM"])
                results["Net_%"] = safe_pct_vec(results["SR_PLUS_NET_SALES"], results["TAM"])
                # [v5.1] Forever numerico (Basic+Premium)
                if "R4_BASIC_GROSS_SALES" in results.columns:
                    results["Forever_Tot_Gross"] = (
                        pd.to_numeric(results["R4_BASIC_GROSS_SALES"], errors="coerce").fillna(0)
                        + pd.to_numeric(results["R4_PREMIUM_GROSS_SALES"], errors="coerce").fillna(0)
                    )
                else:
                    results["Forever_Tot_Gross"] = 0
                st.markdown(f"**{len(results)} store trovati**")
                # [v5.1] Include STORE_ADDRESS e Forever totale
                rc = {
                    ...
                    "SR_PLUS_NET_SALES": "Net", "Net_%": "Net%",
                    "Activeforever": "Forever (0/1)",
                    "Forever_Tot_Gross": "Forever Tot",
                }
                # Solo colonne presenti
                rc_present = {k: v for k, v in rc.items() if k in results.columns}
                rd = results[list(rc_present.keys())].rename(columns=rc_present).sort_values("Gross%")
...
with tab_rs:
    ...
    rs_display_cols = {
        ...
        "N_PDV": "N PDV",
        "Negozi": "Negozi",                          # [v5.1] lista negozi
        "Citta": "Città",                            # [v5.1] città
        "Tipi_Store": "Tipo",
        "AM": "AM",
        "DM": "DM",
        "Regioni": "Regioni",
        ...
        "Net": "Net",
        "Net_%": "Net %",
        "Forever_Active": "Forever Stores",           # n. store con almeno una Forever
        "Forever_Tot_Gross": "Forever Tot (Gross)",   # [v5.1] NUMERICO Basic+Premium
        "Forever_Tot_Net": "Forever Tot (Net)",       # [v5.1] NUMERICO Basic+Premium
    }
    # Solo le colonne effettivamente presenti
    rs_display_cols = {k: v for k, v in rs_display_cols.items() if k in rs_filtered.columns}
    ...
    if rs_list:
        ...
        if rs_selected:
            ...
            # [v5.1] Calcolo % vettoriale (più veloce e robusto)
            stores_rs["Gross_%"] = gross_pct_vec(
                stores_rs["SR_PLUS_GROSS_SALES"], stores_rs["TAM"], stores_rs["Activeforever"], calc_mode)
            stores_rs["Net_%"] = net_pct_vec(
                stores_rs["SR_PLUS_NET_SALES"], stores_rs["TAM"], stores_rs["Activeforever"], calc_mode)

            # [v5.1] Forever numerico per singolo store
            if "R4_BASIC_GROSS_SALES" in stores_rs.columns:
                stores_rs["Forever_Tot_Gross"] = (
                    pd.to_numeric(stores_rs["R4_BASIC_GROSS_SALES"], errors="coerce").fillna(0)
                    + pd.to_numeric(stores_rs["R4_PREMIUM_GROSS_SALES"], errors="coerce").fillna(0)
                )
            else:
                stores_rs["Forever_Tot_Gross"] = 0

            rs_row = rs_filtered[rs_filtered["COMPANY_NAME"] == rs_selected].iloc[0]

            # Banner: numero store ed eventuale alert se la tabella ne mostrasse meno
            n_pdv_real = len(stores_rs)
            n_pdv_aggregato = int(rs_row["N_PDV"])
            st.markdown(
                f"<div style='background:rgba(0,168,224,0.12);border-left:3px solid #00A8E0;"
                f"padding:0.6rem 0.9rem;border-radius:6px;margin:0.6rem 0;'>"
                f"🏪 <b>{n_pdv_real} punti vendita</b> per questa ragione sociale "
                f"(aggregato: {n_pdv_aggregato})"
                f"</div>",
                unsafe_allow_html=True,
            )
            col_d1, col_d2, col_d3, col_d4, col_d5 = st.columns(5)
            col_d1.metric("PDV", n_pdv_real)
            col_d2.metric("TAM totale", f"{int(rs_row['TAM']):,}")
            col_d3.metric("Gross / Net", f"{int(rs_row['Gross'])} / {int(rs_row['Net'])}")
            col_d4.metric("Forever stores", int(rs_row["Forever_Active"]))
            col_d5.metric("Forever totali (Gross)", int(rs_row.get("Forever_Tot_Gross", 0)))

            # [v5.1] Colonne store: aggiunto STORE_ADDRESS + Forever_Tot
            store_cols = ["STORE", "STORE_ADDRESS", "CITY", "PROVINCE_CODE", "STORE_TYPE",
                          "AREA_MANAGER", "DISTRICT_MANAGER",
                          "TAM", "SR_PLUS_GROSS_SALES", "Gross_%",
                          "SR_PLUS_NET_SALES", "Net_%", "Activeforever", "Forever_Tot_Gross"]
            store_cols = [c for c in store_cols if c in stores_rs.columns]
            store_rename = {
                "STORE": "Store", "STORE_ADDRESS": "Indirizzo",
                "CITY": "Città", "PROVINCE_CODE": "Prov",
                "STORE_TYPE": "Tipo", "AREA_MANAGER": "AM", "DISTRICT_MANAGER": "DM",
                "TAM": "TAM", "SR_PLUS_GROSS_SALES": "Gross", "Gross_%": "Gross%",
                "SR_PLUS_NET_SALES": "Net", "Net_%": "Net%",
                "Activeforever": "Forever (0/1)", "Forever_Tot_Gross": "Forever Tot",
            }
...
with tab4:
    ...
    <div style="text-align:center;color:#7B97BF;font-size:0.75rem;padding:1rem;">
        Wind3 Reload Dashboard v5.1 · Ambassador {zona_label} · {mese_str}<br>
...
# ── TAB CHIUSURE PERSONALIZZATE ─────────────────────────────────
with tab_chiusure:
    render_chiusure_tab(
        file_bytes=file_bytes,
        ambassador=zona_label,
        regioni=regioni,
        sheet_name=sheet_name,
        filter_mode=filter_mode,
    )
